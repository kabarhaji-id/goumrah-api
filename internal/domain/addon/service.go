...
func (s Service) Create(req CreateRequest) (Response, error) {
	...
	if err := s.uow.Do(context.Background(), func(ctx context.Context, db database.DB) error {
		...
		addonCategoryRepository := addon_category.NewRepository(db)
		...
		response = Response{
			Id:        entity.Id,
			Category:  addon_category.Response{},
			CreatedAt: entity.CreatedAt,
			UpdatedAt: entity.UpdatedAt,
			DeletedAt: entity.DeletedAt,
		}

		categoryEntity, err := addonCategoryRepository.FindById(ctx, entity.CategoryId)
		if err != nil {
			return err
		}
		response.Category = addon_category.Response{
