...
func (r Repository) Create(ctx context.Context, entity Entity) (Entity, error) {
	query, args := sqlbuilder.New().
		S(`INSERT INTO "city_tours" ("name", "city", "description", "created_at", "updated_at")`).
		S(`VALUES ($1, $2, $3, NOW(), NOW())`, entity.Name, entity.City, entity.Description).
		S(`RETURNING "id", "name", "city", "description", "created_at", "updated_at"`).
		Build()

	if err := r.db.QueryRow(ctx, query, args...).Scan(
		&entity.Id, &entity.Name, &entity.City, &entity.Description, &entity.CreatedAt, &entity.UpdatedAt,
	); err != nil {
		return Entity{}, err
	}
...
func (r Repository) FindAll(ctx context.Context, opt RepositoryFindAllOption) ([]Entity, error) {
	builder := sqlbuilder.New().
		S(`SELECT "id", "name", "city", "description", "created_at", "updated_at"`).
		S(`FROM "city_tours"`).
...
func (r Repository) Update(ctx context.Context, id int64, entity Entity) (Entity, error) {
	query, args := sqlbuilder.New().
		S(`UPDATE "city_tours"`).
		S(`SET "name" = $1, "city" = $2, "description" = $3, "updated_at" = NOW()`, entity.Name, entity.City, entity.Description).
