...
func (s Service) Create(req CreateRequest) (Response, error) {
	...
	if err := s.uow.Do(context.Background(), func(ctx context.Context, db database.DB) error {
		...
		response = Response{
			Id:          entity.Id,
			Avatar:      null.NewValue(image.Response{}, false),
			Name:        entity.Name,
			Type:        entity.Type,
		...
		if entity.AvatarId.Valid {
			imageEntity, err := imageRepository.FindById(ctx, entity.AvatarId.Int64)
			if err != nil {
				return err
			}

			response.Avatar = null.NewValue(image.Response{
				Id:        imageEntity.Id,
				Src:       imageEntity.Src,
				Alt:       imageEntity.Alt,
