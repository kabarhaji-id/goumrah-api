...
func (r Repository) Create(ctx context.Context, entity Entity) (Entity, error) {
	query, args := sqlbuilder.New().
		S(`INSERT INTO "facilities" ("name", "icon", "created_at", "updated_at")`).
		S(`VALUES ($1, $2, NOW(), NOW())`, entity.Name, entity.Icon).
		S(`RETURNING "id", "name", "icon", "created_at", "updated_at"`).
		Build()

	if err := r.db.QueryRow(ctx, query, args...).Scan(
		&entity.Id, &entity.Name, &entity.Icon, &entity.CreatedAt, &entity.UpdatedAt,
	); err != nil {
		return Entity{}, err
	}
...
func (r Repository) FindAll(ctx context.Context, opt RepositoryFindAllOption) ([]Entity, error) {
	builder := sqlbuilder.New().
		S(`SELECT "id", "name", "icon", "created_at", "updated_at"`).
		S(`FROM "facilities"`).
...
func (r Repository) Update(ctx context.Context, id int64, entity Entity) (Entity, error) {
	query, args := sqlbuilder.New().
		S(`UPDATE "facilities"`).
		S(`SET "name" = $1, "icon" = $2, "updated_at" = NOW()`, entity.Name, entity.Icon).
