...
func (v Validator) ValidateCreateRequest(req CreateRequest) (CreateRequest, error) {
	...
	if iconLength := len(req.Icon); iconLength < 1 {
		return CreateRequest{}, errorx.NewValidationError("icon", constant.ErrMustBeFilled)
	} else if iconLength > 100 {
		return CreateRequest{}, errorx.NewValidationError("icon", constant.ErrMax100Chars)
	}
...
func (v Validator) ValidateUpdateRequest(req UpdateRequest) (UpdateRequest, error) {
	...
	if iconLength := len(req.Icon); iconLength < 1 {
		return UpdateRequest{}, errorx.NewValidationError("icon", constant.ErrMustBeFilled)
	} else if iconLength > 100 {
		return UpdateRequest{}, errorx.NewValidationError("icon", constant.ErrMax100Chars)
	}
