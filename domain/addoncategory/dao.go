...
func (r dao) Insert(tx pgx.Tx, request Request) (Response, error) {
	...
	query, args := queryBuilder.
		InsertInto("addon_categories").
		Cols("name", "created_at", "updated_at").
		Values(request.Name, "NOW()", "NOW()").
		Returning("id", "name", "created_at", "updated_at", "deleted_at").
		Build()

	response := Response{}
	if err := tx.QueryRow(context.Background(), query, args...).Scan(
		&response.Id, &response.Name, &response.CreatedAt, &response.UpdatedAt, &response.DeletedAt,
	); err != nil {
		return Response{}, err
	}
...
func (r dao) Update(tx pgx.Tx, id int64, request Request) (Response, error) {
	...
	query, args := queryBuilder.
		...
		SQL("RETURNING id, name, created_at, updated_at, deleted_at").
		Build()
