...
func handleError(c *fiber.Ctx, err error) error {
	...
	if errors.As(err, &pgError) {
		if pgError.Code == "23503" && pgError.ConstraintName == "guides_avatar_id_fkey" {
			return api.ErrInvalidRequestField(c, "avatar", "Not found")
		}
		if pgError.Code == "23505" && pgError.ConstraintName == "guides_name_unique" {
...
func CreateHandler(c *fiber.Ctx) error {
	...
	// Select avatar if not null from database
	if response.AvatarId.Valid {
		avatar, err := image.Dao.SelectById(tx, response.AvatarId.Int64)
		if err != nil {
			tx.Rollback(context.Background())
		...
		response.Avatar = null.ValueFrom(avatar)
	}
...
func GetAllHandler(c *fiber.Ctx) error {
	...
	// Select all guides from database
	...
	for i, response := range responses {
		if response.AvatarId.Valid {
			...
			responses[i].Avatar = null.ValueFrom(avatar)
		}
	}

	// Count all guides from database
