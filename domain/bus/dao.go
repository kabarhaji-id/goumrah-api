...
func (r dao) Insert(tx pgx.Tx, request Request) (Response, error) {
	...
	query, args := queryBuilder.
		InsertInto("buses").
		Cols("name", "seat", "created_at", "updated_at").
		Values(request.Name, request.Seat, "NOW()", "NOW()").
		Returning("id", "name", "seat", "created_at", "updated_at", "deleted_at").
		Build()

	response := Response{}
	if err := tx.QueryRow(context.Background(), query, args...).Scan(
		&response.Id, &response.Name, &response.Seat, &response.CreatedAt, &response.UpdatedAt, &response.DeletedAt,
	); err != nil {
		return Response{}, err
	}
...
func (r dao) SelectAll(tx pgx.Tx, paginationQuery api.PaginationQuery) ([]Response, error) {
	...
	query, args := queryBuilder.
		Select("id", "name", "seat", "created_at", "updated_at", "deleted_at").
		From("buses").
		OrderBy("id ASC").
...
func (r dao) Update(tx pgx.Tx, id int64, request Request) (Response, error) {
	...
	query, args := queryBuilder.
		...
		Set(
			...
			queryBuilder.Assign("seat", request.Seat),
		...
		SQL("RETURNING id, name, seat, created_at, updated_at, deleted_at").
		Build()
