...
func main() {
	...
	airlineMapper := mapper.NewAirlineMapper(imageRepository, imageMapper)
	embarkationMapper := mapper.NewEmbarkationMapper()
	packageMapper := mapper.NewPackageMapper(imageRepository, imageMapper, packageRepository)
	addonCategoryMapper := mapper.NewAddonCategoryMapper()
	guideMapper := mapper.NewGuideMapper(imageRepository, imageMapper)
	airportMapper := mapper.NewAirportMapper()
	busMapper := mapper.NewBusMapper()
	packageSessionMapper := mapper.NewPackageSessionMapper(embarkationRepository, embarkationMapper, packageSessionRepository)
	hotelMapper := mapper.NewHotelMapper()
	facilityMapper := mapper.NewFacilityMapper()
	addonMapper := mapper.NewAddonMapper(addonCategoryRepository, addonCategoryMapper)
	...
	airlineService := service.NewAirlineService(airlineRepository, airlineValidator, airlineMapper)
	embarkationService := service.NewEmbarkationService(embarkationRepository, embarkationValidator, embarkationMapper)
	packageService := service.NewPackageService(packageRepository, packageValidator, packageMapper)
	addonCategoryService := service.NewAddonCategoryService(addonCategoryRepository, addonCategoryValidator, addonCategoryMapper)
	guideService := service.NewGuideService(guideRepository, guideValidator, guideMapper)
	...
	packageSessionService := service.NewPackageSessionService(packageSessionRepository, packageSessionValidator, packageSessionMapper)
	...
	addonService := service.NewAddonService(addonRepository, addonValidator, addonMapper)
	...
	app.Route("/package-sessions", func(router fiber.Router) {
		router.Post("", packageSessionController.CreatePackageSession)
